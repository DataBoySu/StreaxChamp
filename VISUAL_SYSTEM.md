# StreaxChamp Visual System & Design Tokens

**Version**: 1.0
**Status**: DRAFT
**Context**: Reddit Devvit (Inline + Expanded)
**Aesthetic**: "Neo-Retro" — Pixel Art foundation with modern structure.

---

## 1. Visual System Overview

StreaxChamp uses a **strict, token-driven visual system** designed for safe composability.
The system relies on "fake 3D" (pixel depth) rather than shadows/gradients to maintain the retro aesthetic without sacrificing usability.
...
> **Inline Safety**: Text must never be smaller than `10px` (`text-[10px]`) to ensure readability on mobile webframes.
...
**Rules**:
- Only **ONE** Primary CTA per view (in `<FlowFooter>` or Sticky Header).
- "Save & Post" (Irreversible) must visually dominate "Save Draft".
- Disabled states must correspond to `opacity-50` + `cursor-not-allowed`.
...
> **Constraint**: Notifications displace content (relative positioning) or float absolutely within the `FlowShell`. They never trigger a browser alert.
